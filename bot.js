...
client.on('message', msg => {
    ...
    if (command === 'join') {
        ...
        if (team === 'avi') {
            ...
            guild.member(msg.author).add.roles(role).then(msg.channel.send('you joined team avi'));
        }
        if (team === 'angi') {
            const role = msg.guild.roles.cache.find(role => role.name === "天使 分隊 — angi's squad");
            guild.member(msg.author).add.roles(role).then(msg.channel.send('you joined team angi'));
        }
